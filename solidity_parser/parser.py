...
class AstVisitor(SolidityVisitor):
    ...
    def visitThrowStatement(self, ctx):
        return Node(ctx=ctx, type="ThrowStatement")
    ...
    def visitUncheckedStatement(self, ctx):
        return Node(ctx=ctx, type="UncheckedStatement", body=self.visit(ctx.block()))

    def visitRevertStatement(self, ctx):
        return Node(
            ctx=ctx, type="RevertStatement", functionCall=self.visit(ctx.functionCall())
        )
    ...
    def visitAssemblyMember(self, ctx):
        if type(ctx.identifier()) == list:
            ...
            name = ctx.identifier().getText()
        return Node(ctx=ctx, type="AssemblyMember", name=name)
